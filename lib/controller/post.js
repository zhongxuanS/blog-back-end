...
exports.getPostDetail = async (req, res) => {
  try {
    ...
    const result = await PostService.findOne({
      where: {
        [Op.and]:
          Sequelize.where(Sequelize.fn('DATE_FORMAT', Sequelize.col('updated'), '%Y%m%d'),
...
exports.getPostDetailById = async (req, res) => {
  try {
    ...
    Assert.assertHasParameter(body, 'id', ERROR_CODE.PA_NO_ID);

    const id = body.id;
    const result = await PostService.findOne({ where: { id: id } });
    ...
    logger.error('[getPostDetailById]' + error.message);
    res.json(responseHandler.buildFailedRes(error.message));
  }
}

exports.upinsertPost = async (req, res) => {
  try {
    const body = req.body;
    ...
    const yearMonthDay = now.split('/');
    const postPath = `/post/${yearMonthDay[0]}/${yearMonthDay[1]}/${yearMonthDay[2]}/${title}`
    await PostService.upsert({ id: id, content: content, title: title, brief: content.slice(0, 10), postPath: postPath });
    return res.json(responseHandler.buildSuccessRes());
  } catch (error) {
    logger.error('[upinsertPost]' + error.message);
    res.json(responseHandler.buildFailedRes(error.message));
  }
}